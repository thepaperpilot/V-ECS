return {
	...
	update = function(self)
		if not self.loading:isEmpty() then
			for index,c in self.loading:getComponents("LoadStatus"):iterate() do
				local status = c.status
				if status ~= nil then
					if status.currentStep == worldLoadStep.Finished or status.isCancelled then
						self.loading:deleteEntity(index)
					else
						ig.lock()
						...
						if status.currentStep == worldLoadStep.Setup then
							ig.pushStyleColor(styleColors.PlotHistogram, self.colorLerp(0, 120, 0))
							ig.progressBar(0)
							ig.popStyleColor(1)
							ig.text("Setting up...")
						elseif status.currentStep == worldLoadStep.PreInit then
							local totalNodes = 0
							local totalCompleteNodes = 0
							for _,systemStatus in pairs(status:getSystems()) do
							...
							for _,rendererStatus in pairs(status:getRenderers()) do
							...
							ig.pushStyleColor(styleColors.PlotHistogram, self.colorLerp(0, 120, 1/4 + (totalCompleteNodes / totalNodes) / 4))
							ig.progressBar(1/4 + (totalCompleteNodes / totalNodes) / 4)
							ig.popStyleColor(1)
						...
						elseif status.currentStep == worldLoadStep.Init then
							local totalNodes = 0
							local totalCompleteNodes = 0
							for _,systemStatus in pairs(status:getSystems()) do
							...
							ig.pushStyleColor(styleColors.PlotHistogram, self.colorLerp(0, 120, 2/4 + (totalCompleteNodes / totalNodes) / 4))
							ig.progressBar(2/4 + (totalCompleteNodes / totalNodes) / 3)
							ig.popStyleColor(1)
						...
						elseif status.currentStep == worldLoadStep.PostInit then
							local totalNodes = 0
							local totalCompleteNodes = 0
							for _,systemStatus in pairs(status:getSystems()) do
								if systemStatus.postInitStatus ~= functionStatus.NotAvailable then
									totalNodes = totalNodes + 1
									if systemStatus.postInitStatus == functionStatus.Complete then
										totalCompleteNodes = totalCompleteNodes + 1
									end
								end
							end
							for _,rendererStatus in pairs(status:getRenderers()) do
								if rendererStatus.postInitStatus ~= functionStatus.NotAvailable then
									totalNodes = totalNodes + 1
									if rendererStatus.postInitStatus == functionStatus.Complete then
										totalCompleteNodes = totalCompleteNodes + 1
									end
								end
							end
							ig.pushStyleColor(styleColors.PlotHistogram, self.colorLerp(0, 120, 3/4 + (totalCompleteNodes / totalNodes) / 4))
							ig.progressBar(3/4 + (totalCompleteNodes / totalNodes) / 4)
							ig.popStyleColor(1)
							ig.text("Post-Initializing...")
							ig.spacing()
							...
							for _,systemStatus in pairs(status:getSystems()) do
								-- Note we don't draw anything for this system if its init function is not available
								if systemStatus.postInitStatus == functionStatus.Waiting then
									ig.pushStyleColor(styleColors.Text, vec4.new(.4, .4, .4, 1))
									ig.text(systemStatus.name)
									ig.popStyleColor(1)
								elseif systemStatus.postInitStatus == functionStatus.Active then
									ig.pushStyleColor(styleColors.Text, vec4.new(1, 1, 1, 1))
									ig.text(systemStatus.name)
									ig.popStyleColor(1)
								elseif systemStatus.postInitStatus == functionStatus.Complete then
									ig.pushStyleColor(styleColors.Text, vec4.new(0, 1, 0, 1))
									ig.text(systemStatus.name)
									ig.popStyleColor(1)
								end
							...
							for _,rendererStatus in pairs(status:getRenderers()) do
								ig.setCursorPos(loadingWidth / 2, ig.getCursorPos().y)
								-- Note we don't draw anything for this renderer if its init function is not available
								if rendererStatus.postInitStatus == functionStatus.Waiting then
									ig.pushStyleColor(styleColors.Text, vec4.new(.4, .4, .4, 1))
									ig.text(rendererStatus.name)
									ig.popStyleColor(1)
								elseif rendererStatus.postInitStatus == functionStatus.Active then
									ig.pushStyleColor(styleColors.Text, vec4.new(1, 1, 1, 1))
									ig.text(rendererStatus.name)
									ig.popStyleColor(1)
								elseif rendererStatus.postInitStatus == functionStatus.Complete then
									ig.pushStyleColor(styleColors.Text, vec4.new(0, 1, 0, 1))
									ig.text(rendererStatus.name)
									ig.popStyleColor(1)
								end
							...
							ig.pushStyleColor(styleColors.PlotHistogram, self.colorLerp(0, 120, 1))
							ig.progressBar(1)
							ig.popStyleColor(1)
							ig.text("Finishing...")
						end
	...
	colorLerp = function(startHue, endHue, t)
